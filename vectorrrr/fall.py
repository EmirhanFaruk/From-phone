...
class Ball:
    ...
    def ricochet(self):
        ...
        if self.y + self.r + 1 > monitor_height:
            self.y = self.y - self.r - 1 - monitor_height 
            #self.vector[1] = - abs(self.vector[1])
            #self.degree = 360 - self.degree
            #self.change_vector()
            #self.speed += self.speed/100
...
for i in range(10):
    balls.append(Ball(randint(100, monitor_width-100), randint(100, monitor_height-100), 1, [0, 0], 40, randint(1, 355), 0, (255, 255, 255)))#(randint(100, 255), randint(100, 255), randint(100, 255))))#(randint(70, 170), randint(170, 255), randint(100, 200))))#(120, 250, 150)))
